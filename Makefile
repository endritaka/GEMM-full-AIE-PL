...
GRAPH := aie/project.cpp
GRAPH_O := libadf.a

KERNEL := PL_tiling.cpp
KERNEL_XO := PL_tiling.xo
CONFIG_FILE := system.cfg
KERNEL_NAME := PL_tiling
...
# put all AIE related flags here
AIE_FLAGS := $(AIE_INCLUDE_FLAGS) --platform $(BASE_PLATFORM) -workdir=./Work -log-level=1 -v --no-init=false --stacksize=1024 --heapsize=1024 --xlopt=1 -nodot-graph=false -Xchess=main:darts.xargs=-nb --Xmapper=BufferOptLevel8
...
VPP_XO_FLAGS := -c --platform $(BASE_PLATFORM) -t $(TARGET) --save-temps -g --optimize 3 --hls.clock 400000000:$(KERNEL_NAME) 

# --clock.defaultFreqHz 300000000
# --clock.defaultFreqHz 250000000
VPP_LINK_FLAGS := -l -t $(TARGET) --platform $(BASE_PLATFORM) $(KERNEL_XO) $(GRAPH_O) --save-temps -g --optimize 3 --clock.defaultFreqHz 250000000  --config $(CONFIG_FILE) -o $(PFM).xsa
...
# modify here for the aiesim, profile and dump-vcd will increase simulation time by a lot
# maybe remove them
ifeq ($(TARGET),sw_emu)
	$(X86SIM) --pkg-dir=./Work
else
